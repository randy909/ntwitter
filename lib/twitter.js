...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var stream_base = this.options.stream_base,
      idle_connection_timeout = this.options.idle_connection_timeout,
      self = this;
...
//    cancelKeepAliveTimer();
  ...
  stream.on('keep-alive', processKeepAlive);
  ...
  function processKeepAlive () {
    util.print('.');
    cancelKeepAliveTimer();
    self.keep_alive_timer = setTimeout( function () {
      stream.emit('keep-alive-timeout');
